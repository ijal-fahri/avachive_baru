...
<body class="bg-slate-100">
                ...
                <div class="max-w-4xl mx-auto space-y-6">
                    <section class="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div class="flex flex-col items-center text-center md:flex-row md:text-left gap-6">
                            <img src="{{ Auth::user()->profile_photo_path ? asset('storage/' . Auth::user()->profile_photo_path) : 'https://placehold.co/150x150/2dd4bf/1f2937?text=' . strtoupper(substr(Auth::user()->name, 0, 1)) }}" alt="Foto Profil" class="w-24 h-24 rounded-full border-4 border-teal-400 shadow-md object-cover">
                            <div class="profile-info">
                                ...
                                <div class="flex items-center justify-center md:justify-start gap-3 mt-4 flex-wrap">
                                    {{-- PENYESUAIAN 1: Teks tombol diubah --}}
                                    <button id="openProfileModalBtn" class="bg-teal-500 hover:bg-teal-600 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-colors duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500">
    ...
    {{-- MODAL PROFIL (HANYA FUNGSI PASSWORD YANG AKTIF) --}}
    <div id="profileModal" class="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 hidden">
        <div id="profileModalContent" class="modal-content bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative transform scale-95 opacity-0">
            <div class="flex justify-between items-center mb-4 pb-3 border-b border-slate-200">
                {{-- PENYESUAIAN 2: Judul modal diubah --}}
            ...
            <form id="changePasswordForm" action="{{ route('owner.profile.updatePassword') }}" method="POST" class="space-y-4 pt-2">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <img src="{{ Auth::user()->profile_photo_path ? asset('storage/' . Auth::user()->profile_photo_path) : 'https://placehold.co/150x150/e2e8f0/94a3b8?text=...' }}" alt="Foto Profil" class="w-20 h-20 rounded-full object-cover border-2 border-slate-200">
                        <div class="opacity-50">
                            <span class="cursor-not-allowed bg-slate-100 text-slate-800 font-semibold py-2 px-4 rounded-lg text-sm">
                                Pilih Foto
                            </span>
                            <input type="file" class="hidden" disabled>
                            <p class="text-xs text-slate-500 mt-2">Fitur ini akan datang.</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ Auth::user()->name }}" class="block w-full px-3 py-2 border border-slate-300 rounded-md bg-slate-100 cursor-not-allowed" readonly>
                </div>

                <hr class="border-slate-200 !my-6">

                {{-- Fungsi Ubah Password (Aktif) --}}
                <div>
                    <label for="password" class="block text-sm font-medium text-slate-700 mb-1">Password Baru</label>
                    <input type="password" name="password" id="password" class="block w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-400" required>
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-slate-700 mb-1">Konfirmasi Password Baru</label>
                    <input type="password" name="password_confirmation" id="password_confirmation" class="block w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-400" required>
                </div>
                ...
                <div class="flex justify-end pt-4">
                    ...
                    {{-- PENYESUAIAN 3: Teks tombol simpan diubah --}}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if($errors->any())
            ...
            openModal(profileModal, profileModalContent); // Buka kembali modalnya
        @endif
        ...
        document.getElementById('logoutBtn').addEventListener('click', () => {
             Swal.fire({
                 ...
                 showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Ya, Logout!', cancelButtonText: 'Batal'
