...
<body class="bg-slate-100">
            ...
            <header
                ...
                <div class="relative">
                    <button id="profileDropdownBtn"
                        class="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center font-bold text-slate-600 hover:ring-2 hover:ring-teal-400 transition-all">
            ...
            <main class="px-4 md:px-6 pb-28 md:pb-6 mt-8">
                @if (session('success'))
                    <div id="success-alert"
                        class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 rounded-lg max-w-4xl mx-auto"
                        role="alert">
                        <p>{{ session('success') }}</p>
                    </div>
                @endif
                ...
                <div class="max-w-4xl mx-auto space-y-6">
                    <section class="bg-white rounded-2xl shadow-lg p-6">
                        <h3 class="flex items-center gap-3 text-xl font-semibold text-slate-800 mb-4"><i
                                class="bi bi-person-circle text-teal-500"></i>Profil Owner</h3>
                        <div class="flex flex-col items-center text-center md:flex-row md:text-left gap-6">
                            <img src="{{ Auth::user()->profile_photo ? asset('storage/' . Auth::user()->profile_photo) : 'https://placehold.co/150x150/2dd4bf/1f2937?text=' . strtoupper(substr(Auth::user()->name, 0, 1)) }}"
                                alt="Foto Profil"
                            ...
                            <div class="profile-info">
                                ...
                                <div class="flex items-center justify-center md:justify-start gap-3 mt-4 flex-wrap">
                                    <button id="openProfileModalBtn"
                                        class="bg-teal-500 hover:bg-teal-600 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-colors duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500">
                    ...
                    <section class="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div id="infoApp"
                            class="pl-5 border-l-4 border-teal-400 text-slate-600 text-sm leading-relaxed">
                            Avachive adalah sistem manajemen modern yang dirancang untuk membantu Anda mengelola
                            operasional bisnis laundry dengan lebih efisien, dari pemantauan order hingga laporan
                            penjualan.
                        </div>
                    </section>
                    ...
                    <section class="bg-white rounded-2xl shadow-lg p-6">
                        ...
                        <div id="caraPakai"
                            ...
                            <ol class="list-decimal list-inside space-y-2">
                                <li>Gunakan menu <b class="font-semibold text-slate-700">Dashboard</b> untuk melihat
                                    ringkasan bisnis secara cepat.</li>
                                <li>Masuk ke <b class="font-semibold text-slate-700">Manajemen Order</b> untuk menambah,
                                    mengedit, dan melihat status semua pesanan.</li>
                                <li>Kelola cabang dan analisis performa bisnis Anda melalui menu <b
                                        class="font-semibold text-slate-700">Data Cabang</b>.</li>
                                <li>Kelola data admin dan karyawan di halaman <b
                                        class="font-semibold text-slate-700">Data Admin</b> & <b
                                        class="font-semibold text-slate-700">Data Karyawan</b>.</li>
                            </ol>
                        </div>
                    </section>
    ...
    <div id="profileModal"
        ...
        <div id="profileModalContent"
            ...
            <form id="profileUpdateForm" action="{{ route('owner.profile.update') }}" method="POST"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <img id="modalProfilePreview"
                            src="{{ Auth::user()->profile_photo ? asset('storage/' . Auth::user()->profile_photo) : 'https://placehold.co/150x150/e2e8f0/94a3b8?text=...' }}"
                            alt="Foto Profil" class="w-20 h-20 rounded-full object-cover border-2 border-slate-200">
                        <div>
                            <label for="profile_photo"
                                class="cursor-pointer bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold py-2 px-4 rounded-lg text-sm transition-colors">Pilih
                                Foto</label>
                            <input type="file" name="profile_photo" id="profile_photo" class="hidden">
                            <p class="text-xs text-slate-500 mt-2">Opsional. Format: JPG, PNG. Maks 2MB.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end pt-4">
                    <button type="button" id="cancelProfileChangeBtn"
                        class="bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold py-2 px-6 rounded-lg mr-3">Batal</button>
                    <button type="submit"
                        class="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-6 rounded-lg shadow-md">Simpan
                        Perubahan</button>
                </div>
            </form>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (profilePhotoInput && modalProfilePreview) {
                profilePhotoInput.addEventListener('change', function(event) {
                    const file = event.target.files[0];
                    if (file) {
                        modalProfilePreview.src = URL.createObjectURL(file);
                    }
                });
            }
            ...
            @if ($errors->any())
                ...
                openModal(); // Buka kembali modalnya jika ada error
            @endif
