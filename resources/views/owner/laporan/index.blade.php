        ...
        <div class="flex-1 md:ml-64 h-screen overflow-y-auto">
            ...
            <main class="px-4 md:px-6 pb-28 md:pb-6 mt-8">
                ...
                <div class="bg-white p-4 md:p-6 rounded-xl shadow-md mb-6">
                    <form id="filterForm" action="{{ route('owner.laporan.index') }}" method="GET">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
                            <div class="w-full">
                                <label for="cabangFilter" class="text-sm font-medium text-slate-600">Pilih
                                    Cabang</label>
                                <select id="cabangFilter" name="cabang_id"
                                    class="mt-1 block w-full border border-slate-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-teal-400">
                                    <option value="semua">Semua Cabang</option>
                                    @foreach ($cabangs as $cabang)
                                        <option value="{{ $cabang->id }}"
                                            {{ $selectedCabang == $cabang->id ? 'selected' : '' }}>
                            ...
                            <div class="w-full">
                                ...
                                <select id="dateFilter" name="periode"
                                    class="mt-1 block w-full border border-slate-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-teal-400">
                                    <option value="hari_ini" {{ $selectedPeriode == 'hari_ini' ? 'selected' : '' }}>
                                        Hari Ini</option>
                                    <option value="7_hari" {{ $selectedPeriode == '7_hari' ? 'selected' : '' }}>7 Hari
                                        Terakhir</option>
                                    <option value="bulan_ini" {{ $selectedPeriode == 'bulan_ini' ? 'selected' : '' }}>
                                        Bulan Ini</option>
                                </select>
                            </div>
                            <div class="w-full flex flex-wrap gap-2 items-center justify-start md:justify-end">
                                <button type="button" id="btnTambahCabang"
                                    class="px-4 py-2 text-sm font-semibold rounded-lg bg-teal-500 text-white shadow hover:bg-teal-600 transition flex items-center gap-2"><i
                                        class="bi bi-plus-circle"></i> Tambah</button>
                                <button type="button" id="btnDetailCabang"
                                    class="px-4 py-2 text-sm font-semibold rounded-lg bg-slate-700 text-white shadow hover:bg-slate-800 transition flex items-center gap-2 disabled:opacity-50"
                                    disabled><i class="bi bi-eye"></i> Detail</button>
                                <button type="button" id="btnEditCabang"
                                    class="px-4 py-2 text-sm font-semibold rounded-lg bg-slate-700 text-white shadow hover:bg-slate-800 transition flex items-center gap-2 disabled:opacity-50"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-sky-100 pointer-events-none z-0"><i
                                class="bi bi-wallet2 text-8xl"></i></div>
                    ...
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-indigo-100 pointer-events-none z-0"><i
                                class="bi bi-box-seam text-8xl"></i></div>
                        <div class="relative z-10">
                            <p class="text-sm text-slate-500">Jumlah Transaksi</p>
                            <p class="text-2xl font-bold text-slate-800 mt-1">{{ $jumlahTransaksi }}</p>
                        </div>
                    </div>
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-green-100 pointer-events-none z-0"><i
                                class="bi bi-check2-circle text-8xl"></i></div>
                        <div class="relative z-10">
                            <p class="text-sm text-slate-500">Order Selesai</p>
                            <p class="text-2xl font-bold text-slate-800 mt-1">{{ $orderSelesai }}</p>
                        </div>
                    </div>
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-cyan-100 pointer-events-none z-0"><i
                                class="bi bi-person-fill-add text-8xl"></i></div>
                        <div class="relative z-10">
                            <p class="text-sm text-slate-500">Pelanggan Baru</p>
                            <p class="text-2xl font-bold text-slate-800 mt-1">{{ $pelangganBaru }}</p>
                        </div>
                    </div>
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-lime-100 pointer-events-none z-0"><i
                                class="bi bi-trophy text-8xl"></i></div>
                        <div class="relative z-10">
                            <p class="text-sm text-slate-500">Pemasukan Tertinggi</p>
                            <p class="text-2xl font-bold text-green-600 mt-1 truncate"
                                title="{{ $pemasukanTertinggi }}">{{ $pemasukanTertinggi }}</p>
                        </div>
                    </div>
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-red-100 pointer-events-none z-0"><i
                                class="bi bi-graph-down-arrow text-8xl"></i></div>
                        <div class="relative z-10">
                            <p class="text-sm text-slate-500">Pemasukan Terendah</p>
                            <p class="text-2xl font-bold text-red-600 mt-1 truncate"
                                title="{{ $pemasukanTerendah }}">{{ $pemasukanTerendah }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-6">
                        ...
                        <div class="h-64 relative">
                            ...
                            @else
                                <div class="flex items-center justify-center h-full text-slate-400">
                                    <div class="text-center"><i class="bi bi-bar-chart-line text-4xl mb-2"></i>
                    ...
                    <div class="lg:col-span-2 bg-white p-6 rounded-xl shadow-md">
                        <h3 class="font-semibold mb-4 text-slate-700">Layanan Terlaris</h3>
                        <div class="h-64 relative">
                            @if (count($layananTerlaris) > 0)
                                <canvas id="servicesChart"></canvas>
                            @else
                                <div class="flex items-center justify-center h-full text-slate-400">
                                    <div class="text-center"><i class="bi bi-graph-up text-4xl mb-2"></i>
                ...
                <div class="bg-white rounded-xl shadow-md">
                    <div class="p-4 md:p-6 border-b">
                        <h3 class="text-lg font-semibold text-slate-700">Detail Riwayat Transaksi</h3>
                    </div>
                            ...
                            <tbody>
                                @forelse ($transaksiTerbaru as $order)
                                    <tr class="odd:bg-white even:bg-slate-50 hover:bg-teal-50">
                                        <td data-label="ID Transaksi" class="px-6 py-4 font-medium text-slate-800">
                                            <div class="flex items-center">
                                                <span>TRX-{{ \Carbon\Carbon::parse($order->created_at)->format('Ym') }}-{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}</span>
                                                @if (isset($order->is_new) && $order->is_new)
                                                    <span
                                                        class="ml-2 px-2 py-0.5 text-xs font-bold text-white bg-green-500 rounded-full animate-pulse">BARU</span>
                                                @endif
                                            </div>
                                        </td>
                                        <td data-label="Pelanggan" class="px-6 py-4">
                                            {{ $order->pelanggan->nama ?? 'N/A' }}</td>
                                        <td data-label="Cabang" class="px-6 py-4">
                                            {{ $order->cabang->nama_cabang ?? 'N/A' }}</td>
                                        <td data-label="Tgl Masuk" class="px-6 py-4">
                                            {{ \Carbon\Carbon::parse($order->created_at)->format('d M Y') }}</td>
                                        <td data-label="Tgl Selesai" class="px-6 py-4">
                                            {{ strtotime($order->waktu_pembayaran) ? \Carbon\Carbon::parse($order->waktu_pembayaran)->format('d M Y') : ($order->status == 'Selesai' ? \Carbon\Carbon::parse($order->updated_at)->format('d M Y') : '-') }}
                                        </td>
                                        <td data-label="Pengambilan" class="px-6 py-4">
                                            {{ $order->metode_pengambilan ?? 'N/A' }}</td>
                                        <td data-label="Status" class="px-6 py-4">
                                            @php
                                                ...
                                                } elseif ($order->status === 'Sudah Bisa Diambil') {
                                                    $statusClass = 'bg-yellow-100 text-yellow-800';
                                                }
                                            @endphp
                                            <span
                                                class="{{ $statusClass }} px-3 py-1 rounded-full text-xs font-semibold">{{ $order->status }}</span>
                                        </td>
                                        <td data-label="Aksi" class="px-6 py-4 text-center">
                                            <button
                                                class="bg-slate-200 text-slate-800 hover:bg-slate-300 text-xs font-bold py-2 px-3 rounded-lg view-detail-btn"
                                                data-transaction='@json($order)'>
                                                Lihat Detail
                                            </button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center p-4 text-slate-500">Tidak ada transaksi
                                            pada periode ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <div id="modalTambahCabang" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
        <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <form id="formTambahCabang" action="{{ route('owner.cabang.store') }}" method="POST">
                @csrf
                <div class="mb-4"><label for="addNamaCabang"
                        class="block text-sm font-medium text-slate-600 mb-1">Nama Cabang</label><input type="text"
                        name="nama_cabang" id="addNamaCabang" class="w-full border border-slate-300 rounded-lg p-2"
                        required></div>
                <div class="mb-4"><label for="addAlamatCabang"
                        class="block text-sm font-medium text-slate-600 mb-1">Alamat Cabang</label><input
                        type="text" name="alamat" id="addAlamatCabang"
                        class="w-full border border-slate-300 rounded-lg p-2" required></div>
                <div class="mb-4"><label for="addNoWhatsapp"
                        class="block text-sm font-medium text-slate-600 mb-1">No. WhatsApp</label><input
                        type="text" name="no_whatsapp" id="addNoWhatsapp"
                        class="w-full border border-slate-300 rounded-lg p-2" placeholder="Contoh: +6281234567890">
                </div>
                <div class="flex justify-end gap-3 mt-6"><button type="button"
                        class="btn-batal px-4 py-2 text-sm rounded-lg bg-slate-200 hover:bg-slate-300">Batal</button><button
                        type="submit"
                        class="px-4 py-2 text-sm rounded-lg bg-teal-500 text-white hover:bg-teal-600">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    <div id="modalDetailCabang" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
        <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <div id="detailContent" class="space-y-3 text-sm">
                ...
                <p><strong>No. WhatsApp:</strong> <span id="detailNoWhatsapp">Memuat...</span></p>
            </div>
            <div class="flex justify-end gap-3 mt-6">
                <a id="btnHubungiWa" href="#" target="_blank"
                    class="px-4 py-2 text-sm rounded-lg bg-green-500 text-white hover:bg-green-600 flex items-center gap-2"><i
                        class="bi bi-whatsapp"></i> Hubungi</a>
                <a id="btnLihatMaps" href="#" target="_blank"
                    class="px-4 py-2 text-sm rounded-lg bg-slate-700 text-white hover:bg-slate-800 flex items-center gap-2"><i
                        class="bi bi-geo-alt-fill"></i> Lihat di Maps</a>
                <button type="button"
                    class="btn-batal px-4 py-2 text-sm rounded-lg bg-slate-200 hover:bg-slate-300">Tutup</button>
            </div>
        </div>
    </div>
    <div id="modalEditCabang" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
        <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <form id="formEditCabang" action="" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4"><label for="editNamaCabang"
                        class="block text-sm font-medium text-slate-600 mb-1">Nama Cabang</label><input type="text"
                        name="nama_cabang" id="editNamaCabang" class="w-full border border-slate-300 rounded-lg p-2"
                        required></div>
                <div class="mb-4"><label for="editAlamatCabang"
                        class="block text-sm font-medium text-slate-600 mb-1">Alamat Cabang</label><input
                        type="text" name="alamat" id="editAlamatCabang"
                        class="w-full border border-slate-300 rounded-lg p-2" required></div>
                <div class="mb-4"><label for="editNoWhatsapp"
                        class="block text-sm font-medium text-slate-600 mb-1">No. WhatsApp</label><input
                        type="text" name="no_whatsapp" id="editNoWhatsapp"
                        class="w-full border border-slate-300 rounded-lg p-2" placeholder="Contoh: 6281234567890">
                </div>
                <hr class="my-4">
                <div class="flex justify-between items-center mt-6">
                    <button type="button" id="btnHapusCabang"
                        class="px-4 py-2 text-sm rounded-lg bg-red-600 text-white hover:bg-red-700 flex items-center gap-2"><i
                            class="bi bi-trash"></i> Hapus</button>
                    <div class="flex gap-3"><button type="button"
                            class="btn-batal px-4 py-2 text-sm rounded-lg bg-slate-200 hover:bg-slate-300">Batal</button><button
                            type="submit"
                            class="px-4 py-2 text-sm rounded-lg bg-teal-500 text-white hover:bg-teal-600">Simpan
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const setupCharts = () => {
                ...
                if (revCtx && Object.keys(revenueTrendData).length > 0 && Math.max(...Object.values(
                        revenueTrendData)) > 0) {
                    new Chart(revCtx, {
                        type: 'line',
                        data: {
                            labels: Object.keys(revenueTrendData).map(date => dayjs(date).format(
                                'DD MMM')),
                            datasets: [{
                                label: 'Pendapatan',
                ...
                if (servCtx && Object.keys(layananTerlarisData).length > 0) {
                    new Chart(servCtx, {
                        type: 'doughnut',
                        data: {
                            labels: Object.keys(layananTerlarisData),
                            datasets: [{
                                data: Object.values(layananTerlarisData),
                                backgroundColor: ['#2dd4bf', '#60a5fa', '#c084fc', '#f87171',
                                    '#fbbf24'
                                ]
                            }]
                        },
            ...
            const openDetailModal = (trx) => {
                if (!trx) {
                    console.error('Data transaksi tidak valid.');
                    return;
                }

                document.getElementById('modalOrderId').textContent =
                    `TRX-${dayjs(trx.created_at).format('YYYYMM')}-${String(trx.id).padStart(4, '0')}`;
                        ...
                        if (Array.isArray(services) && services.length > 0) {
                            servicesHtml = services.map(s => {
                                ...
                                if (s.diskon && s.diskon > 0) {
                                    diskonInfo =
                                        `<span class="text-xs text-red-500 ml-2">(${s.diskon}% off dari Rp ${parseInt(s.harga_asli).toLocaleString('id-ID')})</span>`;
                                }
                                return `<li>${s.kuantitas || 1}x ${s.nama || 'N/A'} - Rp ${(s.harga ? parseInt(s.harga) : 0).toLocaleString('id-ID')} ${diskonInfo}</li>`;
                            }).join('');
                ...
                const statusBadgeHtml = (status) => {
                    ...
                    else if (status === 'Sudah Bisa Diambil') sClass = 'bg-yellow-100 text-yellow-800';
            ...
            mainContent.addEventListener('click', (e) => {
                ...
                if (detailButton) {
                    try {
                        openDetailModal(JSON.parse(detailButton.dataset.transaction));
                    } catch (e) {
                        Swal.fire('Error', 'Data detail transaksi rusak.', 'error');
                    }
                }
            });
            ...
            setupModal(document.getElementById('modalTambahCabang'), document.getElementById('btnTambahCabang'),
                '.btn-batal');
            setupModal(document.getElementById('modalDetailCabang'), null, '.btn-batal');
            setupModal(document.getElementById('modalEditCabang'), null, '.btn-batal');
            ...
            document.getElementById('btnDetailCabang').addEventListener('click', async () => {
                ...
                if (data) {
                    ...
                    // [PERUBAHAN] Tampilkan nomor WhatsApp
                    const noWa = data.no_whatsapp || '-';
                    document.getElementById('detailNoWhatsapp').textContent = noWa;
                    const btnWa = document.getElementById('btnHubungiWa');
                    if (data.no_whatsapp) {
                        btnWa.href =
                        `https://wa.me/${data.no_whatsapp.replace(/[^0-9]/g, '')}`; // Bersihkan nomor dari spasi/simbol
                        btnWa.style.display = 'inline-flex';
                    } else {
                        btnWa.style.display = 'none';
                    }

                    document.getElementById('btnLihatMaps').href =
                        `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(data.alamat)}`;
                    document.getElementById('modalDetailCabang').classList.remove('hidden');
                }
            });
            ...
            document.getElementById('btnEditCabang').addEventListener('click', async () => {
                ...
                const data = await fetchCabangData(id);
                if (data) {
                    document.getElementById('formEditCabang').action = updateCabangUrlTemplate.replace(
                        'PLACEHOLDER', id);
                    document.getElementById('formHapusCabang').action = destroyCabangUrlTemplate
                    ...
                    // [PERUBAHAN] Isi nomor WhatsApp
                    document.getElementById('editNoWhatsapp').value = data.no_whatsapp || '';
