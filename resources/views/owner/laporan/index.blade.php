            ...
            <main class="px-4 md:px-6 pb-28 md:pb-6 mt-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        ...
                        <div class="relative z-10"><p class="text-sm text-slate-500">Pemasukan Tertinggi</p><p class="text-2xl font-bold text-green-600 mt-1">Rp {{ number_format($pemasukanTertinggi, 0, ',', '.') }}</p></div>
                    </div>
                    <div class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        <div class="absolute -right-4 -bottom-4 text-red-100 pointer-events-none z-0"><i class="bi bi-graph-down-arrow text-8xl"></i></div>
                        <div class="relative z-10"><p class="text-sm text-slate-500">Pemasukan Terendah</p><p class="text-2xl font-bold text-red-600 mt-1">Rp {{ number_format($pemasukanTerendah, 0, ',', '.') }}</p></div>
                    </div>
                </div>
                ...
                try {
                    ...
                    if (Array.isArray(services) && services.length > 0) {
                        servicesHtml = services.map(s => {
                            let diskonInfo = '';
                            // Memeriksa apakah diskon ada dan lebih dari 0
                            if (s.diskon && s.diskon > 0) {
                                diskonInfo = `<span class="text-xs text-red-500 ml-2">(${s.diskon}% off dari Rp ${parseInt(s.harga_asli).toLocaleString('id-ID')})</span>`;
                            }
