...
<body class="bg-slate-100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div
                        ...
                        <div class="relative z-10">
                            ...
                            <p class="text-2xl font-bold text-green-600 mt-1">{{ $pemasukanTertinggi }}</p>
                        </div>
                    </div>
                    <div
                        class="bg-white p-6 rounded-xl shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                        ...
                        <div class="relative z-10">
                            ...
                            <p class="text-2xl font-bold text-red-600 mt-1">{{ $pemasukanTerendah }}</p>
                        </div>
                    </div>
                </div>
                                ...
                                @forelse ($transaksiTerbaru as $order)
                                    <tr class="odd:bg-white even:bg-slate-50 hover:bg-teal-50">
                                        <td data-label="ID Transaksi" class="px-6 py-4 font-medium text-slate-800">
                                            TRX-{{ \Carbon\Carbon::parse($order->created_at)->format('Ym') }}-{{ str_pad($order->id, 4, '0', STR_PAD_LEFT) }}
                                        </td>
    ...
    <div id="modalDetailCabang" class="hidden fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
        <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <div class="flex justify-end gap-3 mt-6"><a id="btnLihatMaps" href="#" target="_blank"
                    class="px-4 py-2 text-sm rounded-lg bg-slate-700 text-white hover:bg-slate-800 flex items-center gap-2"><i
                        class="bi bi-geo-alt-fill"></i> Lihat di Maps</a><button type="button"
                    class="btn-batal px-4 py-2 text-sm rounded-lg bg-slate-200 hover:bg-slate-300">Tutup</button></div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
                    ...
                    try {
                        ...
                        if (Array.isArray(services) && services.length > 0) {
                            servicesHtml = services.map(s =>
                                `<li>${s.kuantitas || 1}x ${s.nama || 'N/A'} - Rp ${(s.harga ? parseInt(s.harga) : 0).toLocaleString('id-ID')}</li>`
                                ).join('');
            ...
            setupModal(document.getElementById('modalTambahCabang'), document.getElementById('btnTambahCabang'),
                '.btn-batal');
            setupModal(document.getElementById('modalDetailCabang'), document.getElementById('btnDetailCabang'),
                '.btn-batal');
            setupModal(document.getElementById('modalEditCabang'), document.getElementById('btnEditCabang'),
                '.btn-batal');
