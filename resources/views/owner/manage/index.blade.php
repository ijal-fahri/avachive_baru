        ...
        <div class="flex-1 md:ml-64 h-screen overflow-y-auto">
            ...
            <main class="px-4 md:px-6 pb-28 md:pb-6 mt-8">
                    ...
                    <form action="{{ route('owner.manage') }}" method="GET">
                        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                            ...
                            <div class="relative w-full md:w-auto">
                                ...
                                <input type="text" name="search" placeholder="Cari ID atau nama..."
                                    value="{{ request('search') }}"
                                    class="pl-10 pr-4 py-2 w-full sm:w-64 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6">
                    <a href="{{ $filterStatus == 'Diproses' ? route('owner.manage') : route('owner.manage', ['status' => 'Diproses']) }}"
                        class="report-card bg-white p-4 rounded-xl shadow-md flex items-center gap-4 cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-1 {{ $filterStatus == 'Diproses' ? 'ring-2 ring-teal-400' : '' }}">
                        <div class="bg-orange-100 p-3 rounded-full"><i
                                class="bi bi-arrow-repeat text-orange-600 text-2xl"></i></div>
                        <div>
                            <p class="text-sm text-slate-500">Order Proses</p>
                            <p class="text-2xl font-bold text-slate-800">{{ $statusCounts['Diproses'] ?? 0 }}</p>
                        </div>
                    </a>
                    <a href="{{ $filterStatus == 'Siap diantar' ? route('owner.manage') : route('owner.manage', ['status' => 'Siap diantar']) }}"
                        class="report-card bg-white p-4 rounded-xl shadow-md flex items-center gap-4 cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-1 {{ $filterStatus == 'Siap diantar' ? 'ring-2 ring-teal-400' : '' }}">
                        <div class="bg-purple-100 p-3 rounded-full"><i class="bi bi-truck text-purple-600 text-2xl"></i>
                        </div>
                        <div>
                            <p class="text-sm text-slate-500">Siap Diantar</p>
                            <p class="text-2xl font-bold text-slate-800">{{ $statusCounts['Siap diantar'] ?? 0 }}</p>
                        </div>
                    </a>
                    <a href="{{ $filterStatus == 'Siap diambil' ? route('owner.manage') : route('owner.manage', ['status' => 'Siap diambil']) }}"
                        class="report-card bg-white p-4 rounded-xl shadow-md flex items-center gap-4 cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-1 {{ $filterStatus == 'Siap diambil' ? 'ring-2 ring-teal-400' : '' }}">
                        <div class="bg-indigo-100 p-3 rounded-full"><i
                                class="bi bi-person-walking text-indigo-600 text-2xl"></i></div>
                        <div>
                            <p class="text-sm text-slate-500">Siap Diambil</p>
                            <p class="text-2xl font-bold text-slate-800">{{ $statusCounts['Siap diambil'] ?? 0 }}</p>
                        </div>
                    </a>
                    <a href="{{ $filterStatus == 'Selesai' ? route('owner.manage') : route('owner.manage', ['status' => 'Selesai']) }}"
                        class="report-card bg-white p-4 rounded-xl shadow-md flex items-center gap-4 cursor-pointer transition-all duration-200 hover:shadow-xl hover:-translate-y-1 {{ $filterStatus == 'Selesai' ? 'ring-2 ring-teal-400' : '' }}">
                        <div class="bg-green-100 p-3 rounded-full"><i
                                class="bi bi-check2-circle text-green-600 text-2xl"></i></div>
                        <div>
                            <p class="text-sm text-slate-500">Order Selesai</p>
                            <p class="text-2xl font-bold text-slate-800">{{ $statusCounts['Selesai'] ?? 0 }}</p>
                        </div>
                    </a>
                </div>
                    ...
                    <div class="table-wrapper overflow-x-auto">
                        <table class="responsive-table w-full text-sm text-left text-slate-500">
                            ...
                            <tbody id="ordersTableBody">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center p-8 text-slate-500">Tidak ada data order
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const statusBadge = (status) => {
                ...
                switch (status) {
                    case 'Diproses':
                        return `<span class="bg-orange-100 text-orange-800 ${baseStyle}">${status}</span>`;
                    case 'Siap diantar':
                        return `<span class="bg-purple-100 text-purple-800 ${baseStyle}">${status}</span>`;
                    case 'Siap diambil':
                        return `<span class="bg-indigo-100 text-indigo-800 ${baseStyle}">${status}</span>`;
                    case 'Selesai':
                        return `<span class="bg-green-100 text-green-800 ${baseStyle}">${status}</span>`;
                    default:
                        return `<span class="bg-slate-100 text-slate-800 ${baseStyle}">${status}</span>`;
                }
            };
                    ...
                    try {
                        ...
                        if (Array.isArray(services) && services.length > 0) {
                            servicesHtml = services.map(s => {
                                const harga = s.harga ? parseInt(s.harga).toLocaleString('id-ID') :
                                    'N/A';
                                return `<li>${s.kuantitas || 1}x ${s.nama || 'Layanan tidak diketahui'} - Rp ${harga}</li>`;
                            }).join('');
                            servicesHtml =
                                `<ul class="list-disc list-inside text-slate-700">${servicesHtml}</ul>`;
                        }
                    } catch (e) {
