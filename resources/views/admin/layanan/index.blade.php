...
<body class="bg-slate-100 text-slate-800 antialiased">
                ...
                <div class="p-4 sm:p-6 pb-28 md:pb-6">
                    <div
                        ...
                        <div class="flex items-center gap-4">
                            <h1 class="text-lg font-semibold text-slate-800">Daftar Layanan Cabang
                    ...
                    <section class="bg-white rounded-2xl shadow-lg p-4 sm:p-6 md:p-8">
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
                            ...
                            <button id="add-btn"
                                class="w-full md:w-auto flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-lg bg-teal-500 text-white shadow-md hover:bg-teal-600 transition-all active:scale-95">
                        ...
                        <div class="p-4 bg-slate-50 border border-slate-200 rounded-lg mb-6">
                            <div id="kategori-filter"
                                ...
                                <div><input type="radio" name="kategori" id="filter-kiloan" value="Kiloan"
                                        class="peer hidden"><label for="filter-kiloan"
                                        class="cursor-pointer px-4 py-1.5 rounded-full peer-checked:bg-teal-500 peer-checked:text-white peer-checked:shadow-md transition-colors duration-200">Kiloan</label>
                                </div>
                                <div><input type="radio" name="kategori" id="filter-satuan" value="Satuan"
                                        class="peer hidden"><label for="filter-satuan"
                                        class="cursor-pointer px-4 py-1.5 rounded-full peer-checked:bg-teal-500 peer-checked:text-white peer-checked:shadow-md transition-colors duration-200">Satuan</label>
                                </div>
                            </div>
                        </div>
    ...
    <div id="formModal"
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-lg relative animate-modal-in">
            ...
            <form id="layananForm" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="nama" class="block mb-1 text-sm font-medium text-slate-700">Nama
                            Layanan</label>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="diskon" class="block mb-1 text-sm font-medium text-slate-700">Diskon
                                (%)</label>
                            <input type="number" name="diskon" id="diskon" placeholder="Contoh: 10"
                                class="form-input">
                        </div>
                    </div>
                </div>
    ...
    @if ($errors->any())
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                $('#nama').val("{{ old('nama') }}");
                $('#paket').val("{{ old('paket') }}");
                $('#kategori').val("{{ old('kategori') }}");
                $('#harga').val("{{ old('harga') }}");
                $('#diskon').val("{{ old('diskon') }}");

                let errorList =
                    "<ul class='mt-2 list-disc list-inside text-sm text-left'>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>";
                Swal.fire({
    ...
    <script>
        $(document).ready(function() {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (row.diskon > 0) {
                                const hargaFinal = data - (data * row.diskon / 100);
                                return `<div class="flex flex-col">
                                        <span class="font-semibold">${formatCurrency(hargaFinal)}</span>
                                        <s class="text-slate-400 text-xs">${formatCurrency(data)}</s>
                                    </div>`;
                            }
            ...
            const layananForm = $('#layananForm');
            const formMethod = $('#formMethod');
            const openModal = () => formModal.removeClass('hidden').addClass('flex');
            const closeModal = () => formModal.removeClass('flex').addClass('hidden');

            $('#add-btn').on('click', () => {
                layananForm[0].reset();
                modalTitle.text('Tambah Layanan Baru');
                layananForm.attr('action', "{{ route('produk.store') }}");
                formMethod.val('POST');
                openModal();
            });
            ...
            $('#layananTable tbody').on('click', '.btn-edit', function() {
                ...
                layananForm.attr('action', `{{ url('admin/produk') }}/${data.id}`);
                formMethod.val('PUT');
                $('#nama').val(data.nama);
            ...
            userMenuButton.on('click', e => {
                e.stopPropagation();
                userMenu.toggleClass('hidden');
            });
            $(window).on('click', e => {
                if (!userMenuButton.is(e.target) && userMenuButton.has(e.target).length === 0 && !userMenu
                    .is(e.target) && userMenu.has(e.target).length === 0) {
